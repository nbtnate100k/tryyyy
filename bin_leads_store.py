...
def _aggregate_bank_state_city() -> tuple[
    ...
    return sort_items(banks), sort_items(states), sort_items(cities)
...
def filter_dimension_picklists(
    *,
    ...
    b, s, c = _aggregate_bank_state_city()
    return {
        "bank": b,
        "state": s,
        "city": c[:max_cities],
...
def pop_n_random_filtered(
    ...
    *,
    ...
    """Random draw from tier pile, optionally filtered by state, BIN, city, bank, and/or card brand (from BIN)."""
    t = norm_stock_tier(tier)
    if n < 1:
        return None
